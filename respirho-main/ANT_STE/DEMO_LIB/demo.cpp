...
#define USER_ANTCHANNEL       (1)
#define USER_DEVICENUM        (5)
#define USER_DEVICETYPE       (2)
#define USER_TRANSTYPE        (1)
#define USER_CHANNELTYPE      (1)      // 0 for master

#define USER_PERIOD           (656)
...
FILE* fp1;
int stato = 0;
int precdisp=0;  //array di flag, dice se il campione è arrivato
int dispreal=0;  //contiene il numero di campioni acquisiti dall'unità 1, 2 e 3 (serve a calcolare il data loss)
int dispteo=0; //contiene il numero di campioni che le unità 1, 2, 3 avrebbero dovuto acquisire 
int started_5;
int cont = 0;
...
BOOL Demo::Init(UCHAR ucDeviceNumberUSB_)
{
	...
	if (ucDeviceNumberUSB_ == 0xFF)
	{
		...
		printf("--------------------------------------------------------------------------------\n");
		printf("La modalita' verbosa e' attivata, si puo' attivare o disattivare in qualsiasi \nmomento (dopo aver scelto il nome del file) premendo v+invio\n");
...
void Demo::Start()
{
	...
	while (!bMyDone)
	{
		...
		case 'g':
		{
			precdisp = 0; dispteo = 0; dispreal = 0; 
			started_5 = 1;
			SYSTEMTIME st;
			GetLocalTime(&st);		
		...
		case 'q':
		{
			...
			printf("Numero campioni ricevuti dall'unita' 5: %d\n", dispreal); 
			fprintf(fp1, "Numero campioni ricevuti dall'unita' 5: %d\n", dispreal);
			
			
			
			printf("Numero campioni teorici unita' 5: %d\n", dispteo); 
			fprintf(fp1, "Numero campioni teorici unita' 5: %d\n",  dispteo);
			
			dif = difftime(end, start);
			printf("L'acquisizone e' durata %.0lf secondi\n", dif);
			fprintf(fp1, "L'acquisizone e' durata %.0lf secondi\n", dif);
			float frequenza = dispteo / dif;
			printf("Frequenza di sampling: %.2lf Hz\n", frequenza); //calcolata con campioni teorici
			fprintf(fp1, "Frequenza di sampling: %.2lf Hz\n", frequenza); 
			float loss1 = (1 - (float)dispreal / dispteo) * 100;
			
			printf("Data loss unita' 5: %.2lf%%\n", loss1);
		
			fprintf(fp1, "Data loss unita' 5: %.2lf%%\n", loss1);
...
void Demo::MessageThread()
{
	...
	while (!bDone)
	{
		if (pclMessageObject->WaitForMessage(100))
		{
...
void Demo::ProcessMessage(ANT_MESSAGE stMessage, USHORT usSize_)
{
	...
	case MESG_RESPONSE_EVENT_ID:
	{
		...
		case MESG_NETWORK_KEY_ID:
		{
			...
			bStatus = pclMessageObject->AssignChannel(USER_ANTCHANNEL, PARAMETER_RX_NOT_TX, 0, MESSAGE_TIMEOUT);
			break;
		}
				...
				if (stato == 1)
				{
					...
					if (cont == 0)
					{
						...
						if (bBroadcasting)
						{
							...
							dispteo++;
							if (precdisp == 1) {
								precdisp = 0;   //il campione è arrivato, azzero il flag
								//pclMessageObject->SendAcknowledgedData(USER_ANTCHANNEL, aucTransmitBuffer, 1);
							}
							else
							{    //il campione non è arrivato, aggiungo il dato mancante, ma solo dopo aver ricevuto il primo campione in assoluto
								if (started_5) 
								{
									fprintf(fp1, "[05],[00],[FF],[00],[00],[00],[00],[00],");
									timestamp();
									printf("[05],[00],[FF],[00],[00],[00],[00],[00]\n");
								}
							}
							// Echo what the data will be over the air on the next message period.
							if (bDisplay && verbose_mode)
   ...
   if (bPrintBuffer)
   {
	   ...
	   {		   
		   if (stato == 1)
		   {
			   ...
			   //dato ricevuto dal dispositivo 5, incrementa il conteggio e attiva flag ricezione
			   if (stMessage.aucData[ucDataOffset + 0] == 5) {
				   precdisp = 1; 
				   started_5 = 0;
				   dispreal++;
			   }
			  
			   fprintf(fp1, "[%02x],[%d],[%02x],[%02x],[%02x],[%02x],[%02x],[%02x],",
