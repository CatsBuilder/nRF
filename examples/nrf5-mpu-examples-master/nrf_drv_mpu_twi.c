 ...
 */
 #define MPU_TWI_SCL_PIN 17 // per ICM 13 (MPU: quando � connessa a nrF52DK: 27; quando � connessa a nrf52832: 17)
 #define MPU_TWI_SDA_PIN 16 // per ICM 12 (MPU: quando � connessa a nrF52DK: 26; quando � connessa a nrf52832: 16)
...
#define MPU_ADDRESS     			0x68 //anche l'indirizzo di ICM20948 � 0x68
#define MPU_AK89XX_MAGN_ADDRESS     0x0C // anche lo slave address di AK09916 � 0Ch
...
#if (defined(ICM20948) || defined(MPU9255)) && (TWI_COUNT >= 1) // Magnetometer only works with TWI so check if TWI is enabled
...
#endif // (defined(ICM20948)) || defined(MPU9255)) && (TWI_COUNT >= 1) // Magnetometer only works with TWI so check if TWI is enabled
